...
// factors of n less than n
self.properFactors = function(n) {
	var primeFactorization = self.primeFactorization(n);
	var factors = [1];
	for (var i = 1; i < primeFactorization.length; i++) {
		factors.push(math.prod(primeFactorization.slice(0, i)));
		factors.push(math.prod(primeFactorization.slice(i, primeFactorization.length)));
	}
...
self.factors = function(n) {
	var factors = self.properFactors(n);
	factors.push(n);
	return factors;
};
